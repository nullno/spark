                   ...
                   var DomEvent = {
                    ...
                    empty:function(){
                             SparkUtil.traverse(this.child,function(item,index){
                              _scope.WidgetCache[item] && _scope.WidgetCache[item].remove('',true);
                             })
            ...
            getNxWidget:function(nxtype,newparams,domtag,defaultcss,defineProperty){
               ...
               if(NEW_WIDGET.type != 'Image' ||  NEW_WIDGET.type != 'Input'){
                   ...
                   NEW_WIDGET.removeChild = _core.getDomEvent.call(NEW_WIDGET,'removeChild');
                    /*清空元素*/ 
                   NEW_WIDGET.empty = _core.getDomEvent.call(NEW_WIDGET,'empty');
                }
            ...
            CarouselWrapper:function(p) {
              !p && (p = {});
                ...
                p.on = {
                  ...
                  hover:function(){
                       var _parentWidget = _scope.getAddressData(this.parentName);
                           ...
                           _parentWidget.stopAutoPlay();
                  },
                  leave:function(){
                       var _parentWidget = _scope.getAddressData(this.parentName);

                        _parentWidget.option.autoPlay && _parentWidget.startAutoPlay();
                ...
                p.displacement = function(p,add,time,activeIndex){
                    ...
                    // 滑动事件
                    if(_parentWidget.on){
                      _parentWidget.on.slideStart && _parentWidget.on.slideStart.call(_parentWidget)
                    }
                    ...
                    var clearTimer = function(){
                         ...
                         if(_parentWidget.on){
                              _parentWidget.on.slideEnd && _parentWidget.on.slideEnd.call(_parentWidget)
                          }
