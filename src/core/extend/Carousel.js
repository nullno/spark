...
const CarouselWrapper = function (p) {
  ...
  p.on = {
    ...
    move: function () {
      ...
      if (this.bounded.y) {
        this.displacementBounce =
          Math.abs(this.position.lastX - this.position.x) >=
          CarouselMain.option.touchRatio * CarouselMain.width();
      }
      if (this.bounded.x) {
        this.displacementBounce =
          Math.abs(this.position.lastY - this.position.y) >=
          CarouselMain.option.touchRatio * CarouselMain.height();
      }
    },
    up: function () {
      ...
      this.displacement(
        {
          ...
          b: {
            x: this.bounded.y
              ? (-this.width() / this.child.length) * activeIndex
              : 0,
            y: this.bounded.x
              ? (-this.height() / this.child.length) * activeIndex
              : 0,
          },
        },
        add,
  ...
  p.displacement = function (p, add, time, activeIndex) {
    ...
    // 滑动事件
    if (CarouselMain.on) {
      CarouselMain.on.slideStart &&
        CarouselMain.on.slideStart.call(CarouselMain);
    }

    if (CarouselMain.option.pagination) {
      CarouselMain.paginationStyle.style = CarouselMain.paginationDefaultStyle;

      CarouselMain.paginationList.getChild(activeIndex).style =
        CarouselMain.paginationActiveStyle;
    }
    ...
    var runDisplacement = function () {
      runTimer = setTimeout(function () {
        ...
        speed =
          parseInt(
            speed > parseInt(TempSpeed * 0.3) ? speed - speed / 4 : speed
          ) + 1;
  ...
  p.style =
    "width:100%;height:100%;position:absolute;background-color:none;overflow:hidden;touch-action:none;";
...
export default function (p) {
  ...
  p.setPagination = function () {
    var _this = this;
    this.paginationStyle = WidgetManager.Css(
      "display:inline-block;width:10px;height:10px;border-radius:10px;margin:5px;background-color:rgba(255,255,255,0.5);"
    );
    ...
    this.paginationList = WidgetManager.List({
      tag: "div",
      style:
        "width:100%;text-align:center;background-color:rgba(0,0,0,0); color:#fff;position:absolute;z-index:0;bottom:0;",
    ...
    Wrapper.displacement(
      {
        ...
        b: {
          x: Wrapper.bounded.y
            ? (-Wrapper.width() / Wrapper.child.length) * activeIndex
            : 0,
          y: Wrapper.bounded.x
            ? (-Wrapper.height() / Wrapper.child.length) * activeIndex
            : 0,
        },
      },
  ...
  p.update = function () {
    ...
    this.style =
      "overflow:hidden;position:relative;width:" + w + "px;height:" + h + "px;";
    ...
    var WrapperWidth =
      this.option.direction == "vertical" ? w : w * Wrapper.child.length;
    var WrapperHeight =
      this.option.direction == "vertical" ? h * Wrapper.child.length : h;

    Wrapper.style =
      "overflow:hidden;position:absolute;width:" +
      WrapperWidth +
      "px;height:" +
      WrapperHeight +
      "px;";
    ...
    SparkUtil.traverse(Wrapper.child, function (name, index, end) {
      ...
      _slide.style =
        ...
        h +
        "px;" +
        (p.option.direction == "vertical" ? "" : "float:left;");
    });

    this.maxIndex = Wrapper.child.length - 1;

    this.option.initSlide =
      this.option.initSlide > this.maxIndex
        ? this.maxIndex
        : this.option.initSlide;
