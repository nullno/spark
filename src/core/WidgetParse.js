...
const WidgetParse = {
    ...
    var DomEvent = {
      ...
      insertFirst: function (newdata, noani) {
        ...
        if (_typeof(newdata, "Array")) {
          ...
          noani
            ? (delete tempWidget.showAni, (tempWidget.style = "animation:none"))
            : "";
      ...
      insert: function (index, newdata, noani) {
        ...
        if (index >= _thisMaxIndex) {
          _thisMaxIndex == 0
            ? _this.insertFirst(newdatas, noani)
            : _this.insertLast(newdatas, noani);
        } else {
          SparkUtil.traverse(newdatas, function (item, index, end) {
            var _widget = _this.item(item, "");
            noani
              ? (delete _widget.showAni, (_widget.style = "animation:none;"))
              : "";
      ...
      update: function (index, newdata) {
        if (
          _typeof(newdata, "Undefined") ||
          _typeof(newdata, "Null") ||
        ...
        this.delete(
          index + 1 >= this.data.length - 1 ? this.data.length - 2 : index + 1,
          1,
          true
        );
      ...
      remove: function (set, noani) {
        ...
        if (
          _typeof(tempWidget.parentName, "String") &&
          !SparkUtil.includes(tempWidget.parentName, "spark-")
        ) {
          if (aniSet && aniSet.ani && !noani) {
            tempWidget.style = "animation:" + aniSet.ani + ";";
            var removeTimer = setTimeout(function () {
      ...
      child: function () {
        ...
        if (this.type == "List") {
          ...
          if (_typeof(this.data, "String")) {
            try {
              tempDataArr = JSON.parse(this.data);
            } catch (err) {
              tempDataArr = [];
          ...
          if (
            _typeof(tempDataArr, "Array") &&
            tempDataArr.length > 0 &&
            _typeof(this.item, "Function")
          ) {
            SparkUtil.traverse(
              tempDataArr,
              function (tempDataItem, index, end) {
  ...
  setDefineProperty: function (address, arr) {
    WidgetParse.WidgetDefineProperty(
      address.indexOf("Css") != -1
        ? Cache.CSSCache[address]
        : GetAddressData(address),
      arr
    );
  },
  getNxWidget: function (
    nxtype,
    newparams,
    domtag,
    defaultcss,
    ...
    NEW_WIDGET.styleObj = CSSManager.cssParse.strStyleToObj(
      WidgetParse.getKeyValue.call(NEW_WIDGET, "style")
    );
    ...
    if (NEW_WIDGET.type === "List") {
      ...
      NEW_WIDGET.insertFirst = WidgetParse.getDomEvent.call(
        NEW_WIDGET,
        "insertFirst"
      );
      NEW_WIDGET.insertLast = WidgetParse.getDomEvent.call(
        NEW_WIDGET,
        "insertLast"
      );
    ...
    if (NEW_WIDGET.type != "Image" || NEW_WIDGET.type != "Input") {
      ...
      NEW_WIDGET.removeChild = WidgetParse.getDomEvent.call(
        NEW_WIDGET,
        "removeChild"
      );
      /*清空元素*/
    ...
    /*指定参数变化监听*/
    defineProperty &&
      WidgetParse.setDefineProperty(NEW_WIDGET.name, defineProperty);
